...
func innerNeedMap(parent string, in map[string]interface{}, out map[string]string) {
	for k, v := range in {
		switch obj := v.(type) {
...
func needSignMap(in map[string]interface{}) map[string]string {
	out := make(map[string]string)
	innerNeedMap("", in, out)
	return out
}

func struct2Map(in any) (map[string]interface{}, error) {
...
func Sign(priv *rsa.PrivateKey, mp map[string]interface{}) (string, error) {
	need := needSignMap(mp)
	return utils.SignMD5WithRSA(priv, concatMap(need))
}

// 返回数据验签
func Verify(pub *rsa.PublicKey, mp map[string]interface{}, sign string) error {
	need := needSignMap(mp)
