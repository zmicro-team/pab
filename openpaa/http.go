...
func (c *Client) post(ctx context.Context, url string, headers map[string]string, body []byte) (*HttpResponse, error) {
	return c.execute(ctx, http.MethodPost, url, headers, body)
}

func (c *Client) execute(ctx context.Context, method, url string, headers map[string]string, body []byte) (*HttpResponse, error) {
	traceId := c.getTraceId(ctx)

	c.log.Debugf(`{"req.proto":{"method":"%s","url":"%s"},"traceId":"%s"}`,
		method, url, traceId)
	...
	c.log.Infof(`{"req":{"method":"%s","url":"%s","header":"%v","body":%s},"traceId":"%s"}`,
		method, url, req.Header, body, traceId)
