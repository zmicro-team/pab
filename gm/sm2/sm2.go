...
func Encrypt(pub *PublicKey, data []byte, random io.Reader, mode int) ([]byte, error) {
	...
	for {
		...
		k, _ = new(big.Int).SetString("68138854824527979349168164105152486823408450232032801777009703772445663572799", 10)
		if err != nil {
			return nil, err
		}
		...
		h := sm3.Sm3Sum(tm)
		c = append(c, h...)
		ct, ok := kdf(length, []byte{0x04}, x2Buf, y2Buf) // 密文
		if !ok {
			continue
		}
		c = append(c, ct...)
...
func Decrypt(priv *PrivateKey, data []byte, mode int) ([]byte, error) {
	...
	c, ok := kdf(length, []byte{0x04}, x2Buf, y2Buf)
	if !ok {
