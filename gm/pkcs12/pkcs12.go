...
func DecodeAll(pfxData []byte, password string) (privateKey any, certificate []*x.Certificate, err error) {
	encodedPassword, err := bmpString(password)
	if err != nil {
		return nil, nil, err
	}
...
func Encode(privateKey any, certificate *x.Certificate, caCerts []*x509.Certificate, password string) (pfxData []byte, err error) {
	encodedPassword, err := bmpString(password)
	if err != nil {
		return nil, err
	}
